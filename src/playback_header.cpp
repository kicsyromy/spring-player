...
PlaybackHeader::PlaybackHeader(GtkBuilder *builder,
            ...
            if (playlist != nullptr)
            {
                ...
                if (track != nullptr)
                {
                        ...
                        state == PlaybackList::PlaybackState::Playing)
                    {
                        ...
                        gtk_label_set_text(self->total_time_,
                                           seconds < 10 ?
                                               fmt::format("{}:0{}", minutes, seconds).c_str() :
                                               fmt::format("{}:{}", minutes, seconds).c_str());
                        gtk_widget_set_visible(gtk_cast<GtkWidget>(self->playback_progress_layout_),
                                               true);
...
void PlaybackHeader::on_play_pause_button_clicked(GtkButton *, PlaybackHeader *self) noexcept
{
    auto playlist = self->playback_list_.lock();
    if (playlist != nullptr)
    {
...
void PlaybackHeader::on_next_button_clicked(GtkButton *, PlaybackHeader *self) noexcept
{
    auto playlist = self->playback_list_.lock();
    if (playlist != nullptr)
    {
...
void PlaybackHeader::on_previous_button_clicked(GtkButton *, PlaybackHeader *self) noexcept
{
    auto playlist = self->playback_list_.lock();
    if (playlist != nullptr)
    {
